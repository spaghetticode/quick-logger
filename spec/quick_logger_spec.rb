Dir.entries(File.join(File.dirname(__FILE__), '../lib')).each do |file|
  if file =~ /.rb$/
    require file
  end
end

describe Quick::Logger do
  class TestLogger < Quick::Logger; end
  ...
  describe 'filename_path' do
    context 'when extending a rails app' do
      before do
        ...
        Rails.stub!(:env => 'test', :root => '/rails/rootpath')
      end
